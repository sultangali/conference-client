...
export const fetchCreate = createAsyncThunk(
  ...
  async (params, { rejectWithValue }) => {
    try {
      const response = await tools.axios.post("/api/todo", params);
...
export const fetchDelete = createAsyncThunk(
  ...
  async ({id}, { rejectWithValue }) => {
    try {
      const response = await tools.axios.delete(`/api/todo/${id}`);
      return response.data;
    } catch (error) {
      if (!error.response) {
        throw error;
...
export const fetchPatch = createAsyncThunk(
  ...
  async ({id, status}, { rejectWithValue }) => {
    try {
      const response = await tools.axios.patch(`/api/todo/${id}`, { status });
      return response.data;
    } catch (error) {
      if (!error.response) {
        throw error;
...
export const fetchUpdate = createAsyncThunk(
  ...
  async ({id, title, description}, { rejectWithValue }) => {
    try {
      const response = await tools.axios.patch(`/api/todo/${id}/update`, { 
        title,
        description
       });
      return response.data;
...
export const fetchAll = createAsyncThunk(
  "auth/fetchAll",
  async () => {
    const { data } = await tools.axios.get(`/api/todo/all`);
    return data;
  }
)
